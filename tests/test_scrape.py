...
from eu_consultations.scrape import (
    scrape, 
    read_consultations_from_json,
    async_scrape
)
...
def test_scraping_creates_output_file(test_output_dir):
    scrape(topic_list=["DIGITAL"], output_folder=test_output_dir, filename=OUTPUT_FILENAME, max_pages=1)
    # list all files
...
def test_scraped_data_conforms_to_dataclass(test_output_dir):
    consultations = scrape(
        topic_list=["DIGITAL"], output_folder=test_output_dir, filename=OUTPUT_FILENAME, max_pages=1)
    stored_consultations = read_consultations_from_json(os.path.join(test_output_dir,OUTPUT_FILENAME))
    shutil.rmtree(test_output_dir)
...
def test_full_pipeline_runs(test_output_dir):
    ...
    consultation_data = asyncio.run(async_scrape(topic_list=["DIGITAL"], max_pages=1, max_feedback = 5,stream_out_folder=scrape_folder))
    ...
    assert set(consultation_ids) == set(after_download_consultation_ids) == set(after_extraction_consultation_ids)
