...
def download_consultation_files(consultation_data: List[Consultation], output_folder: str | os.PathLike) -> List[Consultation]:
            ...
            if feedback:
                for attachment in feedback.attachments:
                    downloaded_filepath = download_file(
                        ...
                        filepath=os.path.join(output_folder,f"{attachment.document_id}_{attachment.file_name}")
                    )
...
def extract_text_from_attachments(
    ...
    converter: DocumentConverter = DocumentConverter()) -> List[Consultation]:
